...
inputName2 = "21cylindrov"
inputName3 = "2cylindre1"
plotName = "21cyl-vs-1cyl-vs-2cyl"

data = np.load("./Results/"+inputName+".npz")
data2 = np.load("./Results/"+inputName2+".npz")
data3 = np.load("./Results/"+inputName3+".npz")
inputData=np.load("./Inputs/"+inputName+".npz")
inputData2=np.load("./Inputs/"+inputName2+".npz")
inputData2=np.load("./Inputs/"+inputName3+".npz")
...
def plotabsbetam (i):
    ...
    for m in [0]:
        j=m+mmax
        j2=m+mmax2
        plt.plot (  frequencies, np.abs( beta[:,0,j]) , '-', label=r"$N=1$")
        plt.plot (  frequencies, np.abs( beta3[:,0,j2]) , '-', label=r"$N=2$, $i=0$" )
        plt.plot (  frequencies, np.abs( beta2[:,i,j2]) , '-', label=r"$N=21$, $i=10$" )
