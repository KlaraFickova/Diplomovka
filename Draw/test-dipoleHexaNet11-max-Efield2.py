...
inputName = "test-dipoleHexaNet11a"
fileName = inputName + "-Efield2"
plotName = inputName + "-Efield2"
...
def Complex2RGB (z, rmid) :
    ...
    amp = np.abs(z)
    h = hue_mid + hue_width * (np.real(z) / amp) 		#hue = 160° +- 20°
    s = amp / rmid
    s = np.where (s < 0, 0., s)
    s = np.arctan(s)/pi *2
    v = 1.*np.ones_like(h)
...
plt.xlabel (r'$x$')
plt.ylabel (r'$y$')
plt.title (r'$f = 0.350$, $r_{mid}=1$, $R=0.45$, $\epsilon = (1.1 \pm 0.1\mathrm{i})^2$')
