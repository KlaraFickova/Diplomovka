#alpha -1, 0 and 1 for 1 cylinder - absolute value, real part, imaginary part
import numpy as np
...
def plotalpham (i):
    ...
    for m in range(-1,2):
        j=m+mmax
        plt.plot (  frequencies, np.abs( alpha[:,i,j]) , '-', label=r'$m=$ %d'%(m) )
    ...
    for m in range(-1,2):
        j=m+mmax
        plt.plot (  frequencies, np.real( alpha[:,i,j]) , '-', label=r'$m=$ %d'%(m) )
    ...
    for m in range(-1,2):
        j=m+mmax
        plt.plot (  frequencies, np.imag( alpha[:,i,j]) , '-', label=r'$m=$ %d'%(m) )
    plt.xlabel ( r'$f$' )
